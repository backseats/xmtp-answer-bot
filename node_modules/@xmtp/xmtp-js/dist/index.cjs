...
var secp2 = require('@noble/secp256k1');
...
var secp2__namespace = /*#__PURE__*/_interopNamespace(secp2);
...
var buildDirectMessageTopic = (sender, recipient) => {
  const members = [ethers.utils.getAddress(sender), ethers.utils.getAddress(recipient)];
  members.sort();
...
var buildUserContactTopic = (walletAddr) => {
  return buildContentTopic(`contact-${ethers.utils.getAddress(walletAddr)}`);
};
var buildUserIntroTopic = (walletAddr) => {
  return buildContentTopic(`intro-${ethers.utils.getAddress(walletAddr)}`);
};
var buildUserInviteTopic = (walletAddr) => {
  return buildContentTopic(`invite-${ethers.utils.getAddress(walletAddr)}`);
};
...
function dateToNs(date) {
  return Long__default.default.fromNumber(date.valueOf()).multiply(1e6);
}
...
var fromNanoString = (s) => {
  ...
  return nsToDate(Long__default.default.fromString(s));
};
var { b64Decode, b64Encode } = proto.fetcher;
function concat(a, b) {
...
function convertWalletClientToSigner(walletClient) {
  ...
  return {
    ...
    signMessage: async (message2) => {
      const signature6 = await walletClient.signMessage({ message: message2, account });
      return signature6;
    }
  };
}
...
var crypto = webcrypto ?? crypto$1.webcrypto;
...
var bytesToHex = secp2__namespace.utils.bytesToHex;
...
var SignedPrivateKey = class {
  ...
  static async generate(signer) {
    const secp256k1 = {
      bytes: secp2__namespace.utils.randomPrivateKey()
    };
    const createdNs = Long__default.default.fromNumber((/* @__PURE__ */ new Date()).getTime()).mul(1e6);
    const unsigned = new UnsignedPublicKey({
      secp256k1Uncompressed: {
        bytes: secp2__namespace.getPublicKey(secp256k1.bytes)
      },
      createdNs
    });
  ...
  async sign(digest) {
    const [signature6, recovery] = await secp2__namespace.sign(
      digest,
    ...
    return new Signature({
      ecdsaCompact: { bytes: signature6, recovery }
    });
  }
  ...
  async signKey(pub) {
    ...
    const signature6 = await this.sign(digest);
  ...
  validatePublicKey() {
    const generatedPublicKey = secp2__namespace.getPublicKey(this.secp256k1.bytes);
    return equalBytes(
      generatedPublicKey,
...
var PrivateKey = class {
  ...
  static generate() {
    ...
    const timestamp = Long__default.default.fromNumber((/* @__PURE__ */ new Date()).getTime());
...
function ecdsaSignerKey(digest, signature6) {
  const bytes = secp2__namespace.recoverPublicKey(
    digest,
    signature6.bytes,
    signature6.recovery
  );
  return bytes ? new UnsignedPublicKey({
    ...
    createdNs: Long__default.default.fromNumber(0)
  }) : void 0;
}
var Signature = class {
  // eslint-disable-line camelcase
  ...
  getPublicKey(digest) {
    let bytes;
    if (this.ecdsaCompact) {
      bytes = secp2__namespace.recoverPublicKey(
        digest,
...
For more info: https://xmtp.org/signatures/`;
  ...
  static signerKey(key, signature6) {
    const digest = hexToBytes(
      ethers.utils.hashMessage(this.identitySigRequestText(key.bytesToSign()))
    );
    return ecdsaSignerKey(digest, signature6);
  }
  async signKey(key) {
    ...
    const signature6 = new Signature({
      walletEcdsaCompact: {
        bytes: sigBytes,
        recovery: eSig.recoveryParam
      }
    });
    return new SignedPublicKey({ keyBytes, signature: signature6 });
  }
};
...
var MS_NS_TIMESTAMP_THRESHOLD = new Long__default.default(10 ** 9).mul(10 ** 9);
var UnsignedPublicKey = class {
  ...
  verify(signature6, digest) {
    if (!signature6.ecdsaCompact) {
      return false;
    }
    return secp2__namespace.verify(
      signature6.ecdsaCompact.bytes,
      digest,
...
var SignedPublicKey = class extends UnsignedPublicKey {
  ...
  toLegacyKey() {
    ...
    let signature6 = this.signature;
    if (signature6.walletEcdsaCompact) {
      signature6 = new Signature({
        ecdsaCompact: signature6.walletEcdsaCompact
      });
    }
  ...
  static fromLegacyKey(legacyKey, signedByWallet) {
    ...
    let signature6 = legacyKey.signature;
    if (signedByWallet) {
      signature6 = new Signature({
        walletEcdsaCompact: signature6.ecdsaCompact
      });
    }
    return new SignedPublicKey({
...
var PublicKey = class extends UnsignedPublicKey {
  ...
  walletSignatureAddress() {
    ...
    const digest = hexToBytes(
      ethers.utils.hashMessage(WalletSigner.identitySigRequestText(this.bytesToSign()))
    );
...
var SignedEciesCiphertext = class {
  constructor({ eciesBytes, signature: signature6 }) {
    ...
    this.signature = new Signature(signature6);
  ...
  static async create(ecies, signer) {
    ...
    const signature6 = await signer.sign(await sha256(eciesBytes));
    return new SignedEciesCiphertext({ eciesBytes, signature: signature6 });
  }
};
...
var ConversationV1 = class {
  constructor(client, address, createdAt) {
    ...
    this.context = void 0;
    this.peerAddress = ethers.utils.getAddress(address);
    this.client = client;
  ...
  async processEnvelope({
    ...
    const decoded = await MessageV1.fromBytes(message2);
...
var MessageV1 = class extends MessageBase {
  ...
  async decrypt(keystore9, myPublicKeyBundle) {
    const responses = (await keystore9.decryptV1(buildDecryptV1Request([this], myPublicKeyBundle))).responses;
    if (!responses.length) {
  ...
  static fromBytes(bytes) {
    const message2 = proto.message.Message.decode(bytes);
  ...
  static async encode(keystore9, payload, sender, recipient, timestamp) {
    const header = {
      sender,
      recipient,
      timestamp: Long__default.default.fromNumber(timestamp.getTime())
    };
    const headerBytes = proto.message.MessageHeaderV1.encode(header).finish();
    const results = await keystore9.encryptV1({
      requests: [
        {
          recipient,
          headerBytes,
...
var DecodedMessage = class {
  ...
  static fromV1Message(message2, content, contentType, contentBytes, contentTopic, conversation, error, contentFallback) {
  ...
  static fromV2Message(message2, content, contentType, contentTopic, contentBytes, conversation, senderAddress, error, contentFallback) {
...
function conversationReferenceToConversation(reference, client, version2) {
  if (version2 === "v1") {
  ...
  throw new Error(`Unknown conversation version ${version2}`);
}
...
// src/ApiClient.ts
var { MessageApi, SortDirection } = proto.messageApi;
...
var JobRunner = class {
  constructor(jobType, keystore9) {
    this.jobType = jobType;
    this.mutex = new asyncMutex.Mutex();
    this.keystore = keystore9;
  }
  get protoJobType() {
  ...
  async getLastRunTime() {
    const { lastRunNs } = await this.keystore.getRefreshJob(
      proto.keystore.GetRefreshJobRequest.fromPartial({
        jobType: this.protoJobType
      })
    );
    if (lastRunNs.equals(Long__default.default.fromNumber(0))) {
      return void 0;
    }
  ...
  async decodeInvites(envelopes, shouldThrow = false) {
    const { responses } = await this.client.keystore.saveInvites({
      requests: envelopes.map((env) => ({
        payload: env.message,
        timestampNs: Long__default.default.fromString(env.timestampNs),
...
var SealedInvitationHeaderV1 = class {
  ...
  static fromBytes(bytes) {
    return new SealedInvitationHeaderV1(
      proto.invitation.SealedInvitationHeaderV1.decode(bytes)
    );
  }
};
...
var SealedInvitation = class {
  ...
  static async fromEnvelope(env) {
    ...
    const envelopeTime = Long__default.default.fromString(env.timestampNs);
  ...
  static async createV1({
    ...
    const invitationBytes = invitation4.toBytes();
...
var validateObject = (obj, objectFields, arrayFields) => {
  for (const field of objectFields) {
    if (!obj[field]) {
      throw new KeystoreError(
        proto.keystore.ErrorCode.ERROR_CODE_INVALID_INPUT,
...
var topicDataToV2ConversationReference = ({
  invitation: invitation4,
  createdNs,
  peerAddress
}) => ({
  context: invitation4.context,
  topic: invitation4.topic,
  peerAddress,
  createdNs
});
var isCompleteTopicData = (obj) => !!obj.invitation;
var topicDataToMap = (topicMap) => {
...
var buildPersistenceKey = (env, walletAddress) => `xmtp/${env}/${walletAddress}/`;
var INVITE_STORAGE_KEY = "invitations/v1";
var V1_STORAGE_KEY = "conversation-v1/v1";
var V2Store = class {
  constructor(persistence, persistenceKey, initialData = /* @__PURE__ */ new Map()) {
  ...
  validate(topicData) {
    return !!topicData.topic && topicData.topic.length > 0 && isCompleteTopicData(topicData);
  }
  async refresh() {
...
var V1Store = class extends V2Store {
  static async create(persistence) {
  ...
  validate(topicData) {
    return !!(topicData.topic && topicData.topic.length && topicData.peerAddress?.length > 0);
  }
};
var EC = elliptic__default.default.ec;
...
var InMemoryKeystore = class {
  ...
  async decryptV2(req) {
    const responses = await mapAndConvertErrors(
      ...
      async (req2) => {
        ...
        if (!topicData) {
          throw new KeystoreError(
            proto.keystore.ErrorCode.ERROR_CODE_NO_MATCHING_PREKEY,
            "no topic key"
          );
        }
    ...
    const responses = await mapAndConvertErrors(
      ...
      async ({ payload, timestampNs }) => {
        ...
        if (sealed.v1) {
          ...
          const invitation4 = await sealed.v1.getInvitation(this.v2Keys);
          const topicData = {
            invitation: invitation4,
          ...
          toAdd.push({ ...topicData, topic: invitation4.topic });
          return {
  ...
  async createInvite(req) {
    try {
      ...
      const invitation4 = new InvitationV1({
  ...
  async getV2Conversations() {
    const convos = this.v2Store.topics.map(
      (invite) => topicDataToV2ConversationReference(invite)
    );
    convos.sort(
  ...
  async getRefreshJob({
    ...
    return proto.keystore.GetRefreshJobResponse.fromPartial({
      lastRunNs: lastRunTime || Long__default.default.fromNumber(0)
    });
  }
  async setRefreshJob({
    jobType,
  ...
  async getLastRunTime(jobType) {
    ...
    return Long__default.default.fromBytes([...bytes]);
  }
...
// src/keystore/persistence/BrowserStoragePersistence.ts
var BrowserStoragePersistence = class {
  constructor(storage) {
...
// src/keystore/persistence/InMemoryPersistence.ts
var InMemoryPersistence = class extends BrowserStoragePersistence {
  static create() {
...
var buildPersistenceFromOptions = async (opts, keys) => {
  ...
  return new PrefixedPersistence(
    prefix,
    shouldEncrypt ? new EncryptedPersistence(basePersistence, keys.identityKey) : basePersistence
  );
};
...
async function snapRPC(method, codecs, req, meta, snapId) {
  let reqParam = null;
  if (codecs.req) {
...
async function hasMetamaskWithSnaps() {
  ...
  if (typeof ethereum?.detected !== "undefined" && Array.isArray(ethereum.detected)) {
  ...
  if (typeof ethereum?.providers !== "undefined" && Array.isArray(ethereum.providers)) {
...
async function getSnap(snapId, version2) {
  try {
    const snaps = await getSnaps();
    return Object.values(snaps).find(
      (snap) => snap.id === snapId && (!version2 || isSameMajorVersion(snap.version, version2))
    );
  } catch (e) {
...
// src/keystore/SnapKeystore.ts
async function getResponse(method, req, meta, snapId) {
...
function SnapKeystore(walletAddress, env, snapId) {
  ...
  for (const [method, apiDef] of Object.entries(apiDefs)) {
    generatedMethods[method] = async (req) => {
      if (!apiDef.req) {
        return getResponse(method, null, snapMeta, snapId);
      }
      return getResponse(method, req, snapMeta, snapId);
    };
  }
  return {
    ...generatedMethods,
...
var { GetKeystoreStatusResponse_KeystoreStatus: KeystoreStatus2 } = proto.keystore;
var SNAP_LOCAL_ORIGIN = "local:http://localhost:8080";
var SnapKeystoreProvider = class {
...
async function checkSnapLoaded(walletAddress, env, snapId) {
  ...
  if (status === KeystoreStatus2.KEYSTORE_STATUS_INITIALIZED) {
    return true;
  }
  return false;
}
...
// src/snapInfo.json
var version = "1.2.2";
var package2 = "@xmtp/snap";

// src/Client.ts
var { Compression } = proto.content;
...
function defaultOptions(opts) {
  const _defaultOptions = {
    ...
    useSnaps: false,
    basePersistence: isBrowser() ? BrowserStoragePersistence.create() : InMemoryPersistence.create(),
  ...
  if (opts?.useSnaps) {
    opts.keystoreProviders = [
      new SnapKeystoreProvider(`npm:${package2}`, version),
...
var Client = class {
  constructor(publicKeyBundle, apiClient, backupClient, keystore9) {
  ...
  static async create(wallet, opts) {
    ...
    const keystore9 = await bootstrapKeystore(options, apiClient, signer);
    const publicKeyBundle = new PublicKeyBundle(
      await keystore9.getPublicKeyBundle()
    );
    const address = publicKeyBundle.walletSignatureAddress();
    apiClient.setAuthenticator(new KeystoreAuthenticator(keystore9));
    const backupClient = await Client.setupBackupClient(address, options.env);
    const client = new Client(publicKeyBundle, apiClient, backupClient, keystore9);
  ...
  async getUserContact(peerAddress) {
    peerAddress = ethers.utils.getAddress(peerAddress);
...
var CompositeCodec = class {
  ...
  decode(content, codecs) {
    return this.fromProto(
      { composite: proto.composite.Composite.decode(content.content), part: void 0 },
      codecs
    );
  }
